...
import uuid
import datetime
from StoreCommerce.models import (db_Product, db_Mobile, db_Product_Image, db_Wishlist, db_Product_Order,
                                  db_Place_Order, db_Computer, db_Shoes, db_Cloth,
                                  db_Watch, db_TV, db_Accessories, db_Review,
                                  db_Message_Admin, db_Comment)
from Authentication.models import db_Profile, db_Profile_detail

# Create your views here.

Product_view = db_Product.objects.all()
P_image_view = db_Product_Image.objects.all()
Wishlist_view = db_Wishlist.objects.all()
Place_order_view = db_Place_Order.objects.all()
Product_order_view = db_Product_Order.objects.all()
Review_view = db_Review.objects.all()
Comment_view = db_Comment.objects.all()
Message_admin_view = db_Message_Admin.objects.all()
...
Accessories_view = db_Accessories.objects.all()
...
class fullView(TemplateView):
    ...
    def get(self, request, ProductID):
        ...
        Accessories_view = db_Accessories.objects.filter(db_Access_ID=ProductID).first()  #
        Wishlist_view = db_Wishlist.objects.filter(db_Wishlist_Email=user_email).all()  #
        Wishlist_count = db_Wishlist.objects.filter(db_Wishlist_Email=user_email).first()  #
        Comment_view = db_Comment.objects.filter(db_Comment_ID=ProductID).all()  #
        ...
        context = {
            ...
            'view_Tv': TV_view,
            'view_Access': Accessories_view,
            'view_Wish': Wishlist_view,
            'count_Wish': Wishlist_count,
            'obj_Comment': Comment_view,
            'view_Rate': Rate_view,
        }
...
class orderProduct(TemplateView):
    ...
    def get(self, request, OrderProdID):
        ...
        P_image_view = db_Product_Image.objects.filter(db_Image_ID=OrderProdID).first()
        Wishlist_view = db_Wishlist.objects.filter(db_Wishlist_Email=email_get).first()
        ...
        context = {
            'view_Product': Product_view1,
            'view_Image': P_image_view,
            'view_Mobile': Mobile_view,
            "price_show": get_price,
            "order_quantity": order_quantity,
            'count_Wish': Wishlist_view,
        }
    ...
    def post(self, request, OrderProdID):
        ...
        try:
            if db_Place_Order.objects.filter(product_id_order=OrderProdID, email_user=var_user_email).first():
                ...
                order_place.status_order = var_status_place
                ...
                order_place = db_Place_Order.objects.create(product_id_order=OrderProdID,
                                                            ...
                                                            status_order=var_status_place,
                                                            # (passed/pending/on_way/cancel)
...
class delete_place_byAdmin(TemplateView):
    def post(self, request):
        ...
        place_Del = db_Place_Order.objects.filter(product_id_order=var_delete_id, email_user=var_delete_email,
                                                  cash_on=var_delete_cashOn)
...
class update_place_byAdmin(TemplateView):
    def post(self, request):
        if request.method == 'POST':
            ...
            place_Update = db_Place_Order.objects.filter(product_id_order=var_update_id, email_user=var_update_email,
                                                         cash_on=var_update_cashOn).first()
    ...
    def post(self, request):
        ...
        try:
            if db_Wishlist.objects.filter(db_Wishlist_ID=var_wishlist_ID, db_Wishlist_Email=var_wishlist_email).first():
                wishlist_obj = db_Wishlist.objects.filter(db_Wishlist_ID=var_wishlist_ID,
                                                          db_Wishlist_Email=var_wishlist_email).first()
                wishlist_obj.delete()
            else:
...
class search(TemplateView):
    ...
    def get(self, request):
        ...
        if Cate == "All":
            mySearch = db_Product.objects.filter(db_Product_Name__contains=search) | db_Product.objects.filter(
                db_Product_Category__contains=search).values() | db_Product.objects.filter(
                db_Product_Type__contains=search).values() | db_Product.objects.filter(
                db_Product_Brand__contains=search).values() | db_Product.objects.filter(
                db_Product_Description__contains=search).values()
        else:
            mySearch = ((db_Product.objects.filter(db_Product_Name__contains=search) | db_Product.objects.filter(
                db_Product_Type__contains=search).values() | db_Product.objects.filter(
                db_Product_Brand__contains=search).values() | db_Product.objects.filter(
                db_Product_Description__contains=search).values()) & db_Product.objects.filter(
                db_Product_Category__contains=Cate).values())
...
class mobile_Product_admin(TemplateView):
    ...
    def post(self, request):
        if request.method == "POST" and request.POST.get("mobile_Name" != ""):
            ...
            var_mobile_Storage_RAM = request.POST.get('mobile_Storage_RAM1') + "" + request.POST.get(
                'mobile_Storage_RAM2')
            var_mobile_Storage_ROM = request.POST.get('mobile_Storage_ROM1') + "" + request.POST.get(
                'mobile_Storage_ROM2')
            ...
            if (request.POST.get('mobile_Camera') == "Not Available"):
                ...
                var_mobile_Camera = request.POST.get('mobile_Camera') + " : (" + request.POST.get(
                    'mobile_Camera2') + "MP Back )"
                ...
                var_mobile_Camera = request.POST.get('mobile_Camera') + " : (" + request.POST.get(
                    'mobile_Camera3') + "MP Front )"
            elif (request.POST.get('mobile_Camera') == "Without Rotatig Camera, With Front Camera"):
                var_mobile_Camera = request.POST.get('mobile_Camera') + " : (" + request.POST.get(
                    'mobile_Camera3') + "MP Front )"
            else:
                var_mobile_Camera = request.POST.get('mobile_Camera') + " : (" + request.POST.get(
                    'mobile_Camera2') + "MP x " + request.POST.get('mobile_Camera3') + "MP)"
            ...
            var_mobile_Resolution = request.POST.get('mobile_Resolution1') + " x " + request.POST.get(
                'mobile_Resolution2') + " Pixels"
...
class computer_Product_admin(TemplateView):
    ...
    def post(self, request):
        if request.method == 'POST' and request.POST.get('computer_Name') != "":
            ...
            var_mobile_Storage_RAM = request.POST.get('computer_Storage_RAM1') + "" + request.POST.get(
                'computer_Storage_RAM2')
            var_mobile_Storage_ROM = request.POST.get('computer_Storage_ROM1') + "" + request.POST.get(
                'computer_Storage_ROM2')
            ...
            if (request.POST.get('computer_Camera') == "Not Available"):
                ...
                var_mobile_Camera = request.POST.get('computer_Camera') + " : (" + request.POST.get(
                    'computer_Camera2') + "MP Back )"
                ...
                var_mobile_Camera = request.POST.get('computer_Camera') + " : (" + request.POST.get(
                    'computer_Camera3') + "MP Front )"
            elif (request.POST.get('computer_Camera') == "Without Rotatig Camera, With Front Camera"):
                var_mobile_Camera = request.POST.get('computer_Camera') + " : (" + request.POST.get(
                    'computer_Camera3') + "MP Front )"
            else:
                var_mobile_Camera = request.POST.get('computer_Camera') + " : (" + request.POST.get(
                    'computer_Camera2') + "MP x " + request.POST.get('computer_Camera3') + "MP)"
            ...
            var_mobile_Resolution = request.POST.get('computer_Resolution1') + " x " + request.POST.get(
                'computer_Resolution2') + " Pixels"
...
class tv_Product_admin(TemplateView):
    ...
    def post(self, request):
        if request.method == 'POST' and request.POST.get('product_Name') != "":
            ...
            var_product_Resolution = request.POST.get('product_Resolution1') + " x " + request.POST.get(
                'product_Resolution2')
